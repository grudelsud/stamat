...
public class FeatureVector extends Element{
        ...
        private int calculatedClusternumber;  // il numero del Cluster (già calcolato)
        ...
        public FeatureVector(String[] fileLine, boolean isResult) {
                ...
                for (int i = startIndexOfFeatures; i < fileLine.length-1; i++) { // aggiunto il -1
                        String[] temp = fileLine[i].split(":");  // necessita dei cedd in forma x:y
                      //  this.features[i - startIndexOfFeatures] = Float.parseFloat(temp[1]); // e prende la y
                        this.features[i - startIndexOfFeatures] = Float.parseFloat(temp[0]);
                }
        }

        public FeatureVector(String[] fileLine) {  // si usa questo in genere, che richiama quello sopra
